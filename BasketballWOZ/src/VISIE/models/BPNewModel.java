...
//            System.out.println(armChannel.getAnimationName() + " " + newAnimation + " " + modelRoot.getName());
//     //       System.out.println(armChannel.getTime());
//        }
        ...
        if(!armChannel.getAnimationName().equals(newAnimation)){
            ...
            if(armChannel.getAnimationName().equals("standingPose")){
            ...
            else if(newAnimation.equals("preDribble")){
                this.executeAnimation(armChannel, newAnimation, speed, l);
            }
                ...
                if(this.canArmAnimOverride(armChannel.getAnimationName(), newAnimation)){
                   this.executeAnimation(armChannel, newAnimation, speed, l);
                }
...
//        System.out.println(legChannel.getAnimationName() + " " + newAnimation + " " + parentCharacter.getSpeed());
//        }
...
//                clothesLower.reset(true);
                ...
                else if(armChannel.getAnimationName().startsWith("pass")){
                    this.executeAnimation(legChannel, newAnimation, speed, l);
                }
//                else if(armChannel.getAnimationName().startsWith("callForPass") && parentCharacter.getSpeed() == 0){
//                    this.executeAnimation(legChannel, newAnimation, speed, l);
//                }
                else if(this.hasAnimationFinished(2)){
                    this.executeAnimation(legChannel, newAnimation, speed, l);
                }
            }
            else if(super.hasAnimationFinished(2)){
                    this.executeAnimation(legChannel, newAnimation, speed, l);
            }
            else{
...
//        }
    ...
    @Override
    public boolean hasAnimationFinished(int channel){
        ...
        if(cpoints[0] != -1){
            ...
            return super.hasAnimationFinished(channel);
        }
    
    }
    
    //checkpoints where leg channel animations can be stopped
    ...
    public void executeAnimation(AnimChannel channel, String animationName, float speed, LoopMode l){
     ...
     //   System.out.println(clothesUpper.getTime() + " " + clothesUpper.getAnimationName() + " "  + armChannel.getAnimationName() + " "  + armChannel.getTime());
     }
   ...
   //       model.rotate(0, -(float)Math.toRadians(headRotationAngle), 0);
    }
...
//        System.out.println("sddd" + skel.getBone("f_index.01.R").getLocalRotation());
//        System.out.println("sdfff" + skel.getBone("hand.R").getModelSpaceRotation());
...
//        
        ...
        Node handNode = sc.getAttachmentsNode("f_index.01.R");
        handNode.setName("ghostPos");
        
        ballPosNode = new Node();
        handNode.attachChild(ballPosNode);
     //   ballPosNode.setLocalTranslation(0, 1f, 0);
    //    ballPosNode.setLocalRotation(new Quaternion().fromAngles(0, (float)Math.toRadians(90), 0));
    }
