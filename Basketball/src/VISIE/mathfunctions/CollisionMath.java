...
//      Vector3f dirVec2 = new Vector3f((float)(Math.sin(Math.toRadians(rotAng2))), 0, (float)Math.cos(Math.toRadians(rotAng2)));
    ...
    //true if clear
    public static boolean rayCollisionTest(Vector3f origin, Vector3f direction, Character c, ArrayList<BasketballCharacter> opponents, int rayNumber, float minDist){
        
         ArrayList<Ray> rays = new ArrayList<Ray>();
         Ray midRay = new Ray(origin, direction);
         CollisionResults results = new CollisionResults();
         float increment = 0.5f;
         float incrementAngle = Conversions.adjustAngleTo360(c.getFacingDirection() + 90);
        ...
        for(int i = 1; i < rayNumber; i++){
            ...
            Vector3f newOrigin = origin.add(incDiff);
            Ray r = new Ray(newOrigin, direction);
            rays.add(r);
...
//            System.out.println(c.getID() + " " + c.getPosition() + "  " +  rays.get(i).origin + " " + rays.get(i).direction);
//        }
        
        for(int i = 0; i < rays.size(); i++){
            for(int j = 0; j < opponents.size(); j++){
                rays.get(i).collideWith(opponents.get(j).getCharacterMesh().getWorldBound(), results);
                if(results.size() > 0){
                    for(int k = 0; k < results.size(); k++){
                        if(results.getClosestCollision().getDistance() < minDist){
                            return false;
                        }
                    }
                }
