 ...
 * @author Divesh
 */
public class StartInterface extends AbstractAppState implements ScreenController, Runnable{
  ...
  public StartInterface(Main m){ 
      ...
      t = new Thread(this);
      System.out.println("Screen thread created");
      t.start();
  ...
  private void populateDDBoxes(){
    DropDown numPlayer = screen.findNiftyControl("numPlayer", DropDown.class);
    DropDown flatRes = screen.findNiftyControl("flatRes", DropDown.class);
    DropDown gameType = screen.findNiftyControl("gameType", DropDown.class);
    
    for(int i = 1; i < 6; i++){
        numPlayer.addItem(Integer.toString(i));
    }
    
    String np = this.getDataOfInput("numPlayers");    
    String t = this.getDataOfInput("gameType");
    
        numPlayer.selectItem(np);
        ...
        gameType.addItem("Team");
        gameType.addItem("Training (dribbling and moving)");
        gameType.addItem("Training (passing)");
        gameType.addItem("Training (shooting)");
        gameType.addItem("Simulation (attack)");
        gameType.addItem("Imitation game");
        gameType.addItem("Free training without opponents");
        gameType.addItem("Copying game");
        gameType.selectItem(t);
  ...
  private void populateTextBoxes(){
      ...
      String isMP = this.getDataOfInput("isMultiPlayer");
  ...
  private void initializeDisplayDiagram(){
        ...
        int as = Integer.parseInt(this.getDataOfInput("activityScreen"));
        int ms = Integer.parseInt(this.getDataOfInput("mainScreen"));
        if(ms == as){
            diagramArray[ms - 1] = 3;
            
            Element e = screen.findElementByName("disp" + (as - 1));
            PanelCreator pc = new PanelCreator();
            pc.setChildLayout("center");
            pc.setBackgroundColor("#ff00ffff");
            pc.setHeight("100%");
            pc.setWidth("100%");
            ...
            e = screen.findElementByName("disp" + (ms - 1));
            pc = new PanelCreator();
            pc.setChildLayout("center");
  ...
  private void updateGameSettings(){
      ...
      s.append("numPlayers," + screen.findNiftyControl("numPlayer", DropDown.class).getSelection().toString() + System.getProperty("line.separator"));
      s.append("isOffense," + screen.findNiftyControl("offButton", RadioButton.class).isActivated()+ System.getProperty("line.separator"));
      s.append("isMultiPlayer," + screen.findNiftyControl("mpCheck", CheckBox.class).isChecked()+ System.getProperty("line.separator"));
...
//      }
      ...
      s.append("gameType," + screen.findNiftyControl("gameType", DropDown.class).getSelection().toString()+ System.getProperty("line.separator"));
      ...
      for(int i = 0; i < diagramArray.length; i++){
          if(diagramArray[i] == 1){
                s.append("activityScreen," + (i + 1) + System.getProperty("line.separator"));
          }
          else if(diagramArray[i] == 2){
               s.append("mainScreen," + (i + 1) + System.getProperty("line.separator"));
          }
...
//  
//  
  public void setClientWaitingMessage(int numConnected, float timeRemaining){
    Label errorMessage = screen.findNiftyControl("errorMessage", Label.class);
    
    if(numConnected == 0){
        errorMessage.setText("Waiting for clients...");
    }
    else if(numConnected == 1){
        errorMessage.setText("1 player connected... game starting in " + String.format("%.1f", timeRemaining));
    }
    else{
        errorMessage.setText(numConnected + " players connected...");
    }
  }
  ...
  private boolean hasInputErrors(){
      ...
      if(screen.findNiftyControl("mpCheck", CheckBox.class).isChecked()){
...
//      RadioButton training = screen.findNiftyControl("trainingButton", RadioButton.class);
      ...
      DropDown numPlayer = screen.findNiftyControl("numPlayer", DropDown.class);
      RadioButton offButton = screen.findNiftyControl("offButton", RadioButton.class);
      CheckBox multiPlayerCheck = screen.findNiftyControl("mpCheck", CheckBox.class);
      ...
      settings.setOffense(offButton.isActivated());
      settings.setMultiPlayer(multiPlayerCheck.isChecked());
      ...
      if(gameType.getSelectedIndex() == 0){
          settings.setGameType("Team");
          settings.setNumPlayers(Integer.parseInt(numPlayer.getSelection().toString()));
      }
      else if(gameType.getSelectedIndex() == 1) {
          settings.setGameType("Dribbling");
          settings.setNumPlayers(1);
      }
      else if(gameType.getSelectedIndex() == 2){
          settings.setGameType("Passing");
          settings.setNumPlayers(3);
      }
      else if(gameType.getSelectedIndex() == 3){
          settings.setGameType("Shooting");
          settings.setNumPlayers(1);
      }
      else if(gameType.getSelectedIndex() == 4){
          settings.setGameType("Simulation");
          settings.setNumPlayers(Integer.parseInt(numPlayer.getSelection().toString()));
      }
      else if(gameType.getSelectedIndex() == 5){
          settings.setGameType("Imitation");
          settings.setNumPlayers(2);
      }
      else if(gameType.getSelectedIndex() == 6){
          settings.setGameType("Free");
          settings.setNumPlayers(Integer.parseInt(numPlayer.getSelection().toString()));
      }
      
      
      if(multiPlayerCheck.isChecked()){
          TextField tcp = screen.findNiftyControl("TCPPortText", TextField.class);
          TextField udp = screen.findNiftyControl("UDPPortText", TextField.class);
      ...
      settings.setRecording(recordCheck.isChecked());
