...
public class EvilModel extends AnimatedModel {
    

    public EvilModel(Spatial s, int id){
            model = s;
            model.rotate((float)Math.toRadians(90), 0, 0);
            model.setLocalScale(0.3f);
            model.setLocalTranslation(0f,-3.5f, 0f);
            this.applyColours();
            model.setName("Evil Robot " + id + " mesh");
            typeIdentifier = "B";
    }
    ...
    public void applyColours(){
            ...
            for(int i = 0; i < n.getChildren().size(); i++){
                    ...
                    if(i == 0){//body colour
                        red = (float)Math.random() * 0.2f;
                        green = (float)Math.random() * 0.2f;
                        blue = (float)Math.random() * 0.2f;
                    }
                    else if (i == 1){//eye colour
                        red = (float)Math.random() * 0.7f;
                        green = (float)Math.random() * 0.7f;
                        blue = (float)Math.random() * 0.7f;
                    }
                    else if(i == 2){//trim colour
                        red = (float)Math.random() * 0.7f;
                        green = (float)Math.random() * 0.7f;
                        blue = (float)Math.random() * 0.7f;
                    }
    ...
    public void removeUserControl(){
        ...
        Bone b = s.getBone("rightArmJoint");
        b.setUserControl(false);
        b = s.getBone("leftArmJoint");
        b.setUserControl(false);
   }
